...
{
    public class DeadlockAnalyzerModel : PageModel
    {
        private readonly DbaToolService _dbaToolService;
        private readonly IDbContextFactory<ApplicationDbContext> _contextFactory;
        private readonly ILogger<DeadlockAnalyzerModel> _logger;

        public DeadlockAnalyzerModel(
            DbaToolService dbaToolService,
            IDbContextFactory<ApplicationDbContext> contextFactory,
            ILogger<DeadlockAnalyzerModel> logger)
        {
        ...
        {
            if (string.IsNullOrWhiteSpace(DeadlockXml))
            {
                HasError = true;
                ErrorMessage = Language == "de"
                    ? "Bitte einen Deadlock-Graph (XML) einfügen."
                    : "Please paste a deadlock graph (XML).";
                return Page();
            }

            try
            {
                AnalysisResult = await _dbaToolService.AnalyzeDeadlockAsync(DeadlockXml, Language);
                AnalysisComplete = true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Fehler bei der Deadlock-Analyse");
                HasError = true;
                ErrorMessage = Language == "de"
        ...
        {
            if (string.IsNullOrWhiteSpace(DeadlockXml) || string.IsNullOrWhiteSpace(AnalysisResult))
            {
                HasError = true;
            ...
            try
            {
                ...
                {
                    Hash = hash,
                    ExecutionPlan = DeadlockXml,
                    AnalysisResult = AnalysisResult,
                    Language = Language,
                    ToolType = "DeadlockAnalyzer"
                });
